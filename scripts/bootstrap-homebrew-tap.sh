...
RENDERED_CASK="$ROOT_DIR/packaging/homebrew/Casks/record-screen.rb"
OUTPUT_DIR="${1:-}"
...
if [[ -f "$RENDERED_CASK" ]]; then
  cp "$RENDERED_CASK" "$OUTPUT_DIR/Casks/record-screen.rb"
  cp \
    "$SKELETON_DIR/.github/workflows/validate-cask.yml.template" \
    "$OUTPUT_DIR/.github/workflows/validate-cask.yml"
else
  cp \
    "$SKELETON_DIR/Casks/record-screen.rb.template" \
    "$OUTPUT_DIR/Casks/record-screen.rb.template"
  cp \
    "$SKELETON_DIR/.github/workflows/validate-cask.yml.template" \
...
Next steps:
  ...
  2. Copy or replace Casks/record-screen.rb in the tap repo
