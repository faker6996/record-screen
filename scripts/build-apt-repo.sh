...
if [[ -z "$INPUT_DEB" ]]; then
  cat <<'EOF' >&2
Usage:
  scripts/build-apt-repo.sh /path/to/record-screen_<version>_amd64.deb [/path/to/output/apt]
EOF
  exit 1
fi
...
APT_ORIGIN="${APT_ORIGIN:-Record Screen}"
APT_LABEL="${APT_LABEL:-Record Screen}"
APT_SUITE="${APT_SUITE:-stable}"
APT_CODENAME="${APT_CODENAME:-stable}"
APT_COMPONENT="${APT_COMPONENT:-main}"
APT_DESCRIPTION="${APT_DESCRIPTION:-Record Screen APT repository}"
APT_REPO_URL="${APT_REPO_URL:-https://faker6996.github.io/record-screen/apt}"
...
if [[ -n "${APT_GPG_PRIVATE_KEY:-}" ]]; then
  ...
  gpg --batch --yes --armor --export "$KEY_FINGERPRINT" > "$REPO_ROOT/record-screen-archive-keyring.asc"
  gpg --batch --yes --export "$KEY_FINGERPRINT" > "$REPO_ROOT/record-screen-archive-keyring.gpg"
  SIGNED_REPO="true"
fi

INSTALL_SNIPPET='echo "Signing is not configured for this repository yet."'
if [[ "$SIGNED_REPO" == "true" ]]; then
  INSTALL_SNIPPET="curl -fsSL $APT_REPO_URL/record-screen-archive-keyring.asc | sudo gpg --dearmor -o /usr/share/keyrings/record-screen-archive-keyring.gpg
echo \"deb [signed-by=/usr/share/keyrings/record-screen-archive-keyring.gpg] $APT_REPO_URL $APT_SUITE $APT_COMPONENT\" | sudo tee /etc/apt/sources.list.d/record-screen.list
sudo apt update
...
<html lang="en">
  <head>
    ...
    <title>Record Screen APT Repository</title>
  </head>
  <body>
    <h1>Record Screen APT Repository</h1>
